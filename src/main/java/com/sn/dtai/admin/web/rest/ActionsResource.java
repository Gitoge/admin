...
public class ActionsResource {
        ...
        if (actions.getCode() == null || actions.getCode().trim().equals("")) {
            ...
            if (verifCodePages.isPresent()) {
                throw new BadRequestAlertException(
                    "Le code '" + actions.getCode() + "' est déjà utilisé : '" + verifCodePages.get().getCode() + "'",
                    ENTITY_NAME,
    ...
    /**
     * {@code GET  /actions/recherchemotsclés} : get the actions by mots clés.
     *
     * @param pageable the pagination information.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list
     *         of actions in body.
     */
    @GetMapping("/actions/recherchemotsclés")
